...
export async function POST(request: Request) {
  ...
  const found = await northstarRepository.get<ReportRow>(
    northstarSql({ postgres: "SELECT id, status FROM reports WHERE number = $1", sqlite: "SELECT id, status FROM reports WHERE number = ?" }),
    [number],
  );
  if (found?.status === "FINAL") return NextResponse.json({ error: "Final reports are immutable and cannot be changed." }, { status: 409 });
